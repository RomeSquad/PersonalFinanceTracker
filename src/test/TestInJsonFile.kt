...
fun checkDeleteTransactionWithId() {
    ...
    val inJsonFile = TransactionsInJsonSerialization()

    inJsonFile.addTransaction(newMockTransaction())
    inJsonFile.addTransaction(transactionToDelete)
    inJsonFile.deleteTransaction(transactionToDelete.id)

    if (inJsonFile.getAllTransactions().contains(transactionToDelete)) {
